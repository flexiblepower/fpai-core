 ...
 * {@link CommodityProfileElement}) that represent an energy profile over time,
 * without an explicit starting time.
 */
public class CommodityProfile<BU extends Quantity, FU extends Quantity> extends
		AbstractList<CommodityProfileElement<BU, FU>> {
	// /**
	...
	//
	// /**
	// * This Builder can be used as a convenient method for creating an
	// instance
	...
	private final CommodityProfileElement<BU, FU>[] profile;
	...
	public CommodityProfile(Commodity<BU, FU> commodity,
		...
		this.commodity = commodity;
		profile = new CommodityProfileElement[values.length];
		for (int ix = 0; ix < values.length; ix++) {
			profile[ix] = new CommodityProfileElement(commodity, duration,
					values[ix]);
		}
	}

	CommodityProfile(Commodity<BU, FU> commodity,
			CommodityProfileElement<BU, FU>[] profile) {
		this.commodity = commodity;
	 ...
	 */
	public CommodityProfileElement[] getProfile() {
	...
	public Measurable<BU> getValueForOffset(Measurable<Duration> offset) {
		CommodityProfileElement e = getElementForOffset(offset);
	 ...
	 */
	public CommodityProfileElement getElementForOffset(
			Measurable<Duration> offset) {
		double offsetDone = 0;
		double offsetNeeded = offset.doubleValue(Duration.UNIT);

		for (CommodityProfileElement element : profile) {
	...
	public Measurable<BU> getTotal() {
		double value = 0;
		for (CommodityProfileElement part : profile) {
	...
	public Measurable<Duration> getDuration() {
		...
		for (CommodityProfileElement e : getProfile()) {
	...
	public CommodityProfile<BU, FU> subprofile(Measurable<Duration> offset,
		...
		List<CommodityProfileElement<BU, FU>> list = new ArrayList<CommodityProfileElement<BU, FU>>();
		...
		if (subprofileStart < 0) {
			list.add(new CommodityProfileElement<BU, FU>(commodity, Measure
					.valueOf(subprofileStart * -1, SECOND), Measure.valueOf(0,
					commodity.getBillableUnit())));
		}

		for (int i = 0; i < size(); i++) {
			CommodityProfileElement e = get(i);
			...
			if (eEnd > subprofileStart) {
				...
				if (timeToSkip == 0) {
					...
					list.add(new CommodityProfileElement<BU, FU>(commodity, e
							.getDuration(), e.getAmount()));
				} else {
					...
					list.add(new CommodityProfileElement<BU, FU>(commodity,
							Measure.valueOf(timeToKeep, SECOND), Measure
									.valueOf(energyValue,
											commodity.getBillableUnit())));
				}
			}
		...
		double sum = 0;
		for (CommodityProfileElement<BU, FU> e : list) {
			sum += e.getDuration().doubleValue(SECOND);
		}
		double remainder = subprofileDuration - sum;
		if (remainder > 0) {
			list.add(new CommodityProfileElement<BU, FU>(commodity, Measure
					.valueOf(remainder, SECOND), Measure.valueOf(0,
					commodity.getBillableUnit())));
		}

		// return the new profile
		return new CommodityProfile<BU, FU>(commodity,
				list.toArray(new CommodityProfileElement[list.size()]));
	}
	...
	@Override
	public CommodityProfileElement get(int index) {
